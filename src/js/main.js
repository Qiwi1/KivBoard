...
const myUsername = urlParams.get("username") || "Пользователь" + (Math.random() + "").substring(2, 6);
...
function main() {
    ...
    signaling_socket.on("connect", function () {
        ...
        signaling_socket.on("wrongAccessToken", function () {
            if (!accessDenied) {
                accessDenied = true;
                showBasicAlert("Доступ запрещен! Неправильный доступ!");
            }
        });
...
function initWhiteboard() {
    $(document).ready(function () {
        ...
        Object.entries(keybinds).forEach(([key, functionName]) => {
            ...
            if (associatedShortcutFunction) {
                ...
                console.error(
                    "Функция которую вы хотите привязать к клавише:",
                    key,
                    "имя:",
                    functionName,
                    "нельзя!"
        ...
        $("#addImgToCanvasBtn")
            ...
            .click(function () {
                if (ReadOnlyService.readOnlyActive) return;
                showBasicAlert(`Перетащите изображение на доску.<br>
                Или загрузите его: <input type="file" id="manualFileUpload" name="myfile" />`);
            ...
            .click(function () {
                ...
                var webDavHtml = $(
                    "<div>" +
                        ...
                        "</tr>" +
                        "<tr>" +
                        '<td style="font-size: 0.7em;" colspan="3">Примечание. Вы должны генерировать и использовать учетные данные приложения, если на сервере dav / nextcloud активировано 2-ух факторная авторизация</td>' +
                        "</tr>" +
                        "<tr>" +
                        "<td></td>" +
                        '<td colspan="2"><span class="loadingWebdavText" style="display:none;">Сохранение в Webdav, пожалуйста, подождите...</span><button class="modalBtn webdavUploadBtn"><i class="fas fa-upload"></i> Начать загрузку</button></td>' +
                        "</tr>" +
                        "</table>" +
                        "</div>"
                );
            ...
            .click(() => {
                ...
                $("#shareWhiteboardDialogCopyReadOnlyLink")
                    ...
                    .click(() => {
                        ...
                        $("#shareWhiteboardDialogMessage")
                            ...
                            .text("Ссылка для чтения скопирована ✓");
                    });
                ...
                $("#shareWhiteboardDialogCopyReadWriteLink")
                    ...
                    .click(() => {
                        $("#shareWhiteboardDialogMessage")
                            .toggleClass("displayNone", false)
                            .text("Ссылка для редактирования скопирована ✓");
                        urlToClipboard();
                    });
            });
        ...
        $("#myFile").on("change", function () {
            ...
            reader.onload = function (e) {
                try {
                    ...
                    showBasicAlert("Это не JSON файл!");
                }
            };
                        ...
                        reader.onloadend = function () {
                            ...
                            loadingTask.promise.then(
                                function (pdf) {
                                    ...
                                    var modalDiv = $(
                                        "<div>" +
                                            "Страница: <select></select> " +
                                            '<button style="margin-bottom: 3px;" class="modalBtn"><i class="fas fa-upload"></i> Загрузить на доску</button>' +
                                    ...
                                    showBasicAlert(modalDiv, {
                                        header: "Загрузка PDF в картинку",
                                        okBtnText: "Назад",
                                        headercolor: "#0082c9",
                                    });
    ...
    function uploadImgAndAddToWhiteboard(base64data) {
        ...
        $.ajax({
            ...
            error: function (err) {
                showBasicAlert("Не удалось загрузить файл: " + JSON.stringify(err));
            },
        });
    }
    ...
    function saveWhiteboardToWebdav(base64data, webdavaccess, callback) {
        ...
        $.ajax({
            ...
            success: function (msg) {
                showBasicAlert("Успешно!", {
                    headercolor: "#5c9e5c",
                });
                console.log("Изображение загружено в webdav!");
                callback();
            },
            error: function (err) {
                console.error(err);
                if (err.status == 403) {
                    showBasicAlert(
                        "Не удалось подключиться к папке Webdav! Пожалуйста, проверьте учетные данные и пути и попробуйте еще раз!"
                    );
                } else {
                    showBasicAlert("Неизвестная ошибка Webdav! ", err);
                }
                callback(err);
            },
        });
    ...
    window.addEventListener("paste", function (e) {
        ...
        if (e.clipboardData) {
            ...
            if (!imgItemFound && whiteboard.tool != "text" && whiteboard.tool != "stickynote") {
                showBasicAlert(
                    "Пожалуйста, перетащите изображение или PDF в доску."
                );
            }
        }
    });
